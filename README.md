...
## 61일차
어제 제시한 정적 파일 미적용 문제가 nginx 적용으로 해결된다고 함.

gunicorn 적용 후, 그리고 도커 파일 분리 후 nginx적용 전에 같은 문제가 발생하는게 신기해. 

1. 컴포즈 파일 수정 - 배포용 컴포즈에 nginx 설정 추가.
새 컨테이너 nginx를 추가. 
web컨테이너가 작동하면 따라 작동하고, 볼륨은 static과 media 두 개를 갖는다. 포트는 80:80. 전용 폴더를 이용.
nginx의 두 볼륨을 web에도 추가.
expose라는 항목을 web컨테이너에 추가하여 장고에ㅓㅅ 처리한 결과를 8000번 포트로 expose하도록 한다.
- expose가 뭐지?
그리고 db볼륨에 두 볼륨 이름을 추가.

2. nginx폴더를 만들고, 그 안에 하나의 도커파일 생성.
이미 제공되는 nginx 이미지를 사용한다.
내용 : 최신 버전의 nginx이미지를 사용
      기본 설정인 default.conf를 삭제한다.
      내가 지정한 설정인 conf.d를 사용한다.

3. 그리고 2에서 설정한 conf.d(nginx설정 정보.)를 로컬에 새로 만들것
nginx폴더에 nginx.conf파일을 생성.
대략적인 틀인 공통 사항이니 vs code가 제공하는 ai로 1차적으로 생성하고 교재에 맞추어 수정.

각 사항을 분석하자면
upstream 블록 : 뒷단에 나올 서버 그룹 (이름)정의.
- 실제 요청을 처리할 서버 주소 지정(우리는 server 컨테이너이름:포트번호 형식)
server블록 : 
- listen : nginx 웹 서버가 어느 포트로 들어오는 네트워크 요청을 대기(리스닝)할 지 설정. 우리는 기본 http 접속 포트인 80번 포트로 지정함.
- location / : 사용자가 보내는 요청을 처리하는 기본 경로. 다른 '특정' 로케이션 블록(static, media)에 걸린 요청을 제외하고 처리.
- 프록시 패스 http://업스트림 블록에서 지정한 이름 으로 요청 들어온 걸 업스트림 그룹으로 넘긴다는 뜻.
- 그 다음줄은 리버스 프록시 구조 상 실제 요청자인 사용자 ip가 전달이 안됨. 그래서 헤더에 사용자ip 담아서 전송.
- 그리고 사용자의 주소창에 입력된 원래 호스트 도메인/ip정보를 헤더에 담아 장고에 전달
- 장고 서버가 보내는 리다이렉트 명령을 nginx사 임의로 수정할 수 없게 함.
location /static/과 /media/블록 : 정적 파일을 직접 처리
- alias 경로; 형태로 이 요청 들어오면 장고 거치지 않고 nginx컨테이너 내부의 지정 경로에서 파일을 직접 찾아 브라우저로 즉시 응답.

컨테이너 끄고, 다시 켜고, exec으로 web컨테이너에 collectstatic하기
그 후 주소를 128.0.0.1로 변경(뒤의 :8000 지우기)
정상 작동 및 정적 파일 적용 확인.

주소 변경에 따른 site설정 수정은 좀 이따.
일단 커밋

## 62일차
아마존 웹 서비스로 프로젝트 옮기기

클라우드 서비스 - AWS로 옮겨서 운영.
장비 사양이나 전기요금 등 문제를 해결할 수 있음.

추천 방안은 AWS Lightsail(대형 서버의 성능 일부만을 저렴히 빌려 나의 서버로 사용.)
무료 체험판을 사용하기.
...
2. 인스턴스 생성
- 인스턴스 : 하나의 가상 서버.
- 어떤 운영체제의 컴퓨터를 어떤 지역에 둘지 결정.
- 윈도우 서버는 추가비용. 리눅스 계열 선택 추천(우분투!)
- 작은 사이트이니 소액 플랜으로.
- 이름 정하고 생성하기.
- 그 후 이 가상 서버의 터미널에 접속이 가능함.
3. 프로젝트 옮기기
- 깃허브 저장소로 들어기 clone 하기.
- 구체저으로는 clone with https의 주소 복사
- 그리고 가상 서버 터미널에서 git clone 복사한주소 입력
4. 가상 서버 환경 조성
- 가상 서버에 도커 설치
```
sudo apt-get update
...
```
일단 우분투에서 소프트웨어 설치에 사용하는 툴인 apt-get을 최신화
그리고 기존에 서버에 도커 관련 무언가가 남아있지 않도록 청소
```
...
```
그 후 백지 위에 새롭게 도커 설치
우분투 서버 시작 시 도커도 자동 시작되도록 2가지 설정 추가.
정상 설치된것을 확인(버전 확인)
*도커 설치는 시스템에 직접 하기 때문에 프로젝트 폴더로 이동하지 않아도 됨.*
```
sudo apt install docker-compose
```
최종적으로 도커 컴포즈까지 설치하면 종료.

## 63일차
구매에 대해 고민하느라 좀 미뤄짐. 일단 무료 체험판 이용이 가능한지 보자.

free 6month

결재처 등록은 카드 찾고 진행해야 함.

6.30 등록했고 크레딧과 만료일 확인함.
aws 콘솔 접속 성공.

이제 할 일
1. 인스턴스 생성
> 현재 별도의 '시작하기' 기능 없이 콘솔로 자동 이동됨. 따라서 '인스턴스 생성하기'도 안보임.
> 현 위치는 콘솔. lightsail을 검색해서 이동. 
> 팝업 모달에 create instance 진행.
> 라이트세일 인스턴스를 90일간 무료 이용 가능.
> 블루프린트(청사진?)은 앱+os(node.js등의 것)말고 os만으로 가서 우분투 지정.
> 다양한 선택지가 있었는데 우분투 외의 것들이 어떤 특징이 있고 어디서 쓰이는지 알면 좋을 것 같다.
> 인스턴스 플랜든 범요 유형, 듀얼 스택(ipv4, v6를 함께 사용)의 네트워크, 90일 무료이용의 월별 5달러 크기.
2. 데이터 옮기기
